...
function count() {
    let chaine = "Unités : " +'<br>'
    for (const j in minions) {
        chaine += minions[j].owned+" "+minions[j].name +'<br>'
    }
    return (chaine)
}
function displayGPS() {
    document.getElementById("gps").innerHTML = count()+"GOLDS Par Segonde (GPS) Global: "+getGPS() +'<br>'+ "Golds par click: "+click_gold
}
...
function buyMinion(id) {
    if (minions[id].cost <= golds) {
        ...
        minions[id].owned++
        minions[id].cost= Math.ceil(minions[id].cost*1.15)
        evolution(id)
        click_g()
        getGPS()
        displayGPS()
        ...
        alert("Vous n'avez pas assez de GOLDS pour effectuer cette achat")
    }
}
function storageD() {
...
setInterval(function (){storageD();},3000)

function resetGame() {
    if (confirm('Attention la progretion sera perdu et le jeu vas être reinitialisé')){
        localStorage.clear()
        location.reload()
    }
}
